	...
	handleBarCodeScanned = async ({ type, data }) => {
		if(type == 256 || type === 'org.iso.QRCode') {
			try {
				...
				await ApiService.activateKiosk({
					token: data.split('|')[0],
					id: this.state.kiosk.id,
					name: data.split('|')[1]
				});
				Alert.alert('¡Se ha activado el kiosco exitosamente!', `El kiosco ${this.state.kiosk.name} ha sido activado`);
				...
				<View style={[styles.flex1, styles.mainContainer, styles.alignItemFlexStart]}>
					<View style={[styles.title, styles.centerObjects]}>
						<View style={[styles.titleContainer, styles.justifyContentCenter]}>
							<BodyText
								align='center'
								text='Escanea el QR del kiosco para iniciar sesión.'
								color='white'
								weight='medium'
							/>
						</View>
