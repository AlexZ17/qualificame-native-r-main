	...
	deleteKiosk = async (fetchKiosks, clearKiosks) => {
		...
		await ApiService.deleteKiosk(this.state.kiosk.id).then(res => {
			...
			this.setState({
				...
				alertBody: 'No se logró eliminar el kiosco',
				onCancelPressed: () => {
					this.setState({showAlert: false})
				},
				...
				<ScrollView style={[styles.flex1]}>
					...
					<View>
						<InlineInput
							...
							text='Alertarme cuando mi nivel de aprobación sea menor a'
						...
						<InlineInput
							onChange={(value, tag) => this.handleInputChange(value, tag)}
							text='Alertarme cuando mi nivel de aprobación sea mayor a'
						...
						<ArrowIcon 
							...
							onPress={()=> this.props.navigation.navigate('KioskWorkingHoursScreen', {kiosk: this.state.kiosk})}
						/>	
						<ArrowIcon
							bgColor='blue'
							iconName='users'
						...
						<ArrowIcon 
							...
							onPress={()=> this.props.navigation.navigate('KioskActiveSessionsScreen', {kiosk: this.state.kiosk})}
						/>
						<View style={[styles.spacer]} />
						<DeleteKioskContainer>
							{(fetchKiosks, clearKiosks) => (
								<ArrowIcon 
									...
									onPress={()=>{
										this.setState({
											showAlert: true,
											alertHead:'Eliminando kiosco',
											alertBody:'¿Estás seguro de que deseas eliminar el kiosco?',
											cancelText:'Cancelar',
