...
export default class CreateReportScreen extends Component {
	...
	state = {
		...
		//AQUI PROBABLEMENTE DEBAMOS AGREGAR ALGO PARA ESCONDER EL DateTimePicker
			...
			if(this.state.fromEndTime){
				...
				if(this.state.fromStartDate <= date) {
					...
					this.setState({
						showAlert: true,
						alertHead: 'Fecha inválida',
						alertBody: 'No puedes seleccionar una fecha de terminación menor a la fecha de inicio 1t'
					});
...
//TAL VEZ DEBAMOS MODIFICAR AQUI PARA ESCONDER EL DATETIMEPICKER
	hideDateTimePicker = () => {
		this.setState({
			//isDatePickerVisible: false,
			//isTimePickerVisible: false,
	...
	handleShowDate = () => {
		if(this.state.dateStart) {
			...
			else return new Date(2002, 12, 12);
		}
		if(this.state.dateEnd) {
			if(this.state.fromEndDate) return this.state.fromEndDate;
			else return new Date(2005, 12, 12);
		}
	}
			...
			<TouchableWithoutFeedback onPress={() => Keyboard.dismiss()}>
				<View style={styles.container}>
							...
							<View style={[styles.row, styles.spacerInput]}>
								...
								<View style={[styles.flex03, styles.smallLeftMargin]}>
									<TouchableOpacity
										...
										disabled={this.state.fromStartDate == 0}
							...
							<View style={[styles.row, styles.spacerInput]}>
								<View style={styles.flex07}>
									<TouchableOpacity
										...
										disabled={this.state.fromStartTime == 0}
								...
								<View style={[styles.flex03, styles.smallLeftMargin]}>
									<TouchableOpacity
										...
										disabled={this.state.fromEndDate == 0}
					...
					{ this.state.showDatePicker &&
						...
						<DateTimePicker
							//value={new Date()}
							value={this.handleShowTime()}
							mode={'time'}
							//date={this.handleShowTime()}
							//isVisible={this.state.isTimePickerVisible}
							//show= {this.state.showTimePicker}
							//aqui aparecia: onConfirm={this.handleTimePicked}-> NO SE LLAMABA A LA FUNCIÓN JAMÁS, LO MISMO EN ONCANCEL Y ABAJO EN EL TIMEPICKER
