import { qualificameApiAxios, qualificameAuthAxios, qualificameVentasApi } from "qualificame-native/app/utils/Axios";
...
export default {
	...
	logoutDevices(params) {
		const url = '/api/logout_device';
		const method = 'PUT';
		const data = {
			kiosk_devices_ids: params.ids
		}
		return this.makeAuthorizedRequest({ method, url, data });
	},
	...
	getKioskDevices(params) {
		const method = 'GET';
		const url = `/api/kiosks/${params.id}/kiosk_devices`;
		return this.makeAuthorizedRequest({ method, url });
	},
	...
	///kiosks/:kiosk_id/events_aggregations

	eventsKiosksAggregations(id, value, start_date, end_date) {
		const method = 'GET';
		const url = `/api/kiosks/${id}/events_aggregations`;
		//const data = filters ? this.makeQueryParams(filters) : '';
		let params = {
			filters: `value=${value};start_date=${start_date};end_date=${end_date}`
		}
		return this.makeAuthorizedRequest({ method, url, params })
	},
	...
	//get a la api de ventas
	getVentas(params) {
		const method = 'GET';
		const url = 'http://192.168.0.102/apiVentasQualificame/v1/auto/'+params;
		return this.makeAuthorizedRequestVentas({ method, url });
	},

	async makeAuthorizedRequestVentas ( requestData={} ) {
		try {
			let res = await qualificameVentasApi(requestData);
			return res.data;
		} catch (error) {
