...
export default function ReportsList (props) {
	function renderItem ({ item }) {
		return <KioskCard
			...
			addHorizontalMargin
			iconName='calendar'
			onPress={() => NavigationService.navigate('ReportDetailScreen', {
				item: {
					...item,
					name: filterKiosk(item.kiosk_id)
				}
			})}
	...
	return <ApiList
		...
		emptyText={kiosksAllIds.length == 0 ? 'Al parecer todavía no tienes ningun kiosco, crea uno para poder generar un reporte' : 'Todavía no tienes reportes. Presiona el botón para crear uno'}
		color='blue'
	/>
}
